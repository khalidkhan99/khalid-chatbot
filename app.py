...
# ============================================================
# LIGHT STYLING — subtle polish only.
# Primary buttons get the accent color (from config.toml theme);
# secondary/utility buttons stay neutral so the UI doesn't look
# like every element is shouting for attention.
# ============================================================
st.markdown("""
<style>
    button[kind="primary"] {
...
with st.sidebar:
    ...
    if st.session_state.user_api_key:
        ...
        if st.button("Remove saved key", help="Stop using your personal API key", use_container_width=True):
        ...
        for c in convs:
            ...
            with col1:
                if st.button(
                    ...
                    st.rerun()
            with col2:
                if st.button("🗑", key=f"del_{c['id']}", help="Delete this chat"):
    ...
    st.caption("🎨 Light/Dark theme badalne ke liye: **⋮ (top-right) → Settings**")
    ...
    with st.chat_message(msg["role"]):
        ...
        if msg["role"] == "assistant":
            if i in st.session_state.feedback_given:
                st.caption(f"Feedback: {st.session_state.feedback_given[i]}")
            else:
                c1, c2, _ = st.columns([1, 1, 10])
                if c1.button("👍", key=f"up_{i}", help="Good response"):
                    save_feedback(IDENTIFIER, msg["content"], "up")
                    st.session_state.feedback_given[i] = "👍"
                    st.rerun()
                if c2.button("👎", key=f"down_{i}", help="Not helpful"):
...
if active_key is None:
    st.warning("Free message limit khatam ho gayi hai. Chat continue karne ke liye apni **Groq API key** daalein.")
